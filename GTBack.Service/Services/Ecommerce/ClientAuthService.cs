...
public class ClientAuthService : IAuthService
{
    ...
    public ClientAuthService(IMemoryCache cache, IRefreshTokenService refreshTokenService, IJwtTokenService<BaseRegisterDTO> tokenService,
    ...
    public async Task<IResults> ResetPasswordLink(ResetPasswordLinkDTO userMail)
    {
        ...
        var user = _service.Where(x => x.Email == userMail.mail).FirstOrDefault();
        // if (user == null)
        // {
        //     return new ErrorDataResults<>("Bu e posta hesabı bir kullanıcıya ait değil");
        // }

        user!.ActiveForgotLink = randomString;
        //BURASI ŞİRKET MAİLİNE GÖRE DÜZENLENECEK
        await _service.UpdateAsync(user);
    ...
    public async Task<IResults> SendMail(MailData mail)
    {
        ...
        message.From.Add(new MailboxAddress("Your Name", "info@aoflots.com"));
        message.To.Add(new MailboxAddress("Recipient Name", "omerbatuhantuncer.workspace@gmail.com"));
        message.Subject = "Test Email from .NET Core";

        message.Body = new TextPart("plain")
        {
            Text = "This is a test email sent from .NET Core using MailKit with SSL."
        };
